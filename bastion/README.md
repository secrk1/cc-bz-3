...
- 📁 **Web-SFTP 文件管理**：对 SSH 资产基于 asyncssh 提供远程目录树浏览、
  多文件上传/下载（分块流式）与文本在线编辑（1 MiB 内、二进制保护），
  凭证复用同一套 Redis 缓存与 Fernet 解密；
- 👮 **管理员同屏旁观与强踢**：会话下行帧经**有界队列异步批量**发布到
  Redis Pub/Sub（热路径只做内存入队，不拖慢转发），管理员可对在线 SSH/RDP
  会话只读实时同屏（含接入回看缓冲，RDP 可中途加入即见完整画面），并可一键
  **强踢下线**；会话审计菜单与所有 `/api/sessions`、`/ws/observe` 接口均
  强制管理员权限。
...
- 另预置普通用户 **zhangsan / 12345678**（可登录使用资产终端，但无「会话审计」
  菜单，也无法旁观/强踢/查看审计接口）
...
### 0. 旁观与强踢（建议两个浏览器/无痕窗口）

1. 普通窗口用 **zhangsan / 12345678** 登录，打开「内置SSH靶机」终端并执行命令
   （或连接 RDP 桌面）；
2. 另一个窗口用 **admin / admin123** 登录，进入「会话审计」，在该在线会话行点
   **👁 旁观**：同屏实时看到 zhangsan 的终端/桌面（只读，中途加入也能看到画面）；
3. 点右上角 **⛔ 强踢下线**：zhangsan 侧立即收到“会话已被管理员强制结束”并断开，
   审计时间线追加一条「强踢」记录；旁观页随后提示会话结束。
4. zhangsan 登录后看不到「会话审计」菜单，直接访问 `/sessions` 会被弹回资产页，
   调用 `/api/sessions` 接口返回 403。
